...
export class EncryptionService {
                ...
                this.firebase
                    ...
                    .then(() => {
                        this.firebase.activateFetched().then(() => {
                            this.firebase
                                .getValue('subscription_key', 'belair')
                                .then(
                                    (res: any) =>
                                        (this.key = CryptoJS.enc.Base64.parse(
                                            res
                                        ))
                                )
                                // TODO: error handling!
    ...
    public encrypt(data: string): string {
        if (this.key) {
            ...
            const cyphertext_raw = CryptoJS.AES.encrypt(data, this.key, {
                iv: iv,
                mode: CryptoJS.mode.CBC,
            });
            const hmac = CryptoJS.HmacSHA256(
                cyphertext_raw.toString(),
                this.key.toString(CryptoJS.enc.Base64)
            );
            return btoa(
