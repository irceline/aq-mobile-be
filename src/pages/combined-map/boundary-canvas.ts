import { Bounds, bounds, latLng, Point, PolyUtil, TileLayer, tileLayer, Util } from 'leaflet';
...
TileLayer.BoundaryCanvas = TileLayer.WMS.extend({
    options: {
        boundary: null
    },
    ...
    createTile: function (coords, done) {
        ...
        this._drawTileInternal(tile, coords, url, Util.bind(done, null, null, tile));

        return tile;
    },

    _pruneTiles: function () {
        TileLayer.WMS.prototype['_pruneTiles'].call(this);
        ...
        for (c = 0; c < b.length; c++) {
            ...
            for (r = 0; r < b[c].length; r++) {
                ...
                for (p = 0; p < b[c][r].length; p++) {
                    coords = isGeoJSON ? latLng(b[c][r][p][1], b[c][r][p][0]) : b[c][r][p];
    ...
    _getOriginalMercBoundary: function () {
        ...
        if (Util.isArray(this.options.boundary)) { // Depricated: just array of coordinates
        ...
        if (this._mercBoundary[0][0]) {
            this._mercBbox = new Bounds(this._mercBoundary[0][0]);
            for (c = 1; c < this._mercBoundary.length; c++) {
                compomentBbox = new Bounds(this._mercBoundary[c][0]);
    ...
    _getClippedGeometry: function (geom, currentBounds) {
        ...
        for (iC = 0; iC < geom.length; iC++) {
            clippedComponent = [];
            clippedExternalRing = PolyUtil.clipPolygon(geom[iC][0], currentBounds);
            ...
            for (iR = 1; iR < geom[iC].length; iR++) {
                clippedHoleRing = PolyUtil.clipPolygon(geom[iC][iR], currentBounds);
    ...
    _getTileGeometry: function (x, y, z, skipIntersectionCheck) {
        ...
        const mercBoundary = this._getOriginalMercBoundary(),
            ...
            tileBbox = new Bounds(new Point(x * ts / zCoeff, y * ts / zCoeff), new Point((x + 1) * ts / zCoeff, (y + 1) * ts / zCoeff));
    ...
    onRemove: function (map) {
        ...
        if (this.options.trackAttribution) {
            ...
            if (!this._attributionRemoved) {
                const attribution = TileLayer.BoundaryCanvas.prototype.getAttribution.call(this);
...
tileLayer.boundaryCanvas = function (url, options) {
    return new TileLayer.BoundaryCanvas(url, options);
};
